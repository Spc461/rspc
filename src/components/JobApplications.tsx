    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">إدارة ومتابعة طلبات التوظيف للمدرسين والموظفين</p>
        </div>
        <motion.button
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            {[
              ...
              { key: 'teacher', label: 'مدرسين', color: 'bg-blue-100 hover:bg-blue-200 text-blue-800' },
              { key: 'staff', label: 'موظفين', color: 'bg-green-100 hover:bg-green-200 text-green-800' },
            ].map((filterOption) => (
              <motion.button
                key={filterOption.key}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                onClick={() => setSelectedJobType(filterOption.key as any)}
                className={`px-3 py-1 rounded-lg text-sm font-medium transition-all ${filterOption.color} ${
                  selectedJobType === filterOption.key ? 'ring-2 ring-offset-2 ring-[#22b0fc]' : ''
                }`}
              >
                {filterOption.label}
              </motion.button>
                ...
                filteredApplications.map((application, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-800">
                      {application.teachingLanguage || application.staffField}
                    </td>
                    <td className="px-4 py-3 text-sm">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        selectedApplication.jobType === 'teacher'
                          ? 'bg-blue-100 text-blue-800'
                          : 'bg-green-100 text-green-800'
                      }`}>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedApplication.teachingLanguage || selectedApplication.staffField}
                    </p>
                  </div>
                  <div>
